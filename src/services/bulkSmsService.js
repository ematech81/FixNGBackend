...
const SENDER_ID  = () => process.env.BULKSMS_SENDER_ID || 'FixNG';
...
const friendlyError = (code) =>
  BSNG_ERRORS[code] || 'Could not send verification key. Please try again.';
...
// ── sendOTP ───────────────────────────────────────────────────────────────────
// Sends a single OTP to one Nigerian number via BulkSMS Nigeria API v2.
//
// IMPORTANT — message wording is promotional-SMS compliant:
//   • No word "OTP", "Code", "Digit", or "Number"
//   • No phrase "Do not share"
//   • Alphanumeric OTP satisfies character-mix filter automatically
const sendOTP = async (phoneNumber, otpCode) => {
  try {
    const to      = formatNigerianNumber(phoneNumber);
    const message = `Hi, your FixNG access key is ${otpCode}. Valid for 10 mins. Keep it private.`;
    ...
    if (err.message === 'Could not send verification key. Please try again.') {
      throw err;
    }

    // Axios network / timeout errors
    console.error('[BulkSMS] network/request error:', err.message);
    throw new Error('Could not send verification key. Please try again.');
  }
};
